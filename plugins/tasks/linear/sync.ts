...
import { DEFAULT_COLOR, PROJECT_PREFIX_PATTERN } from "../views/manage/shared.js";
...
function priority(value: number): TaskPriority {
  return (["none", "urgent", "high", "medium", "low"] as const)[value] ?? "none";
}
...
function issueNumber(issue: LinearIssue): number {
  ...
  if (!match || match[1] !== issue.team.key) {
    throw new LinearMappingError(`Linear issue identifier ${issue.identifier} cannot be mapped safely`);
  }
  const value = Number(match[2]);
  if (!Number.isSafeInteger(value) || value < 1)
    throw new LinearMappingError(`Linear issue identifier ${issue.identifier} has an invalid number`);
  return value;
}

function safeError(error: unknown): { code: LinearSyncErrorCode; message: string } {
  ...
  if (
    ...
    const candidate = error as { code: "LINEAR_RATE_LIMITED" | "LINEAR_API_ERROR"; message?: string };
    return { code: candidate.code, message: candidate.message ?? "Linear synchronization failed" };
  }
  return { code: "LINEAR_API_ERROR", message: "Linear synchronization failed" };
}

export function createLinearSyncService(deps: LinearSyncDependencies): LinearSyncService {
  ...
  const mapState = (issue: LinearIssue): TaskStatus => {
    ...
    if (!warnedStateIds.has(issue.state.id)) {
      ...
      deps.warn?.(`Unknown Linear workflow state ${issue.state.id}; using backlog`);
    }
    return "backlog";
  };
  ...
  function validateSnapshot(issues: readonly LinearIssue[]): void {
    ...
    for (const issue of issues) {
      if (ids.has(issue.id)) throw new LinearMappingError(`Linear returned duplicate issue ${issue.id}`);
      ids.add(issue.id);
      if (!PROJECT_PREFIX_PATTERN.test(issue.team.key))
        throw new LinearMappingError(`Linear team key ${issue.team.key} is not a valid Tasks prefix`);
      issueNumber(issue);
    }
    const teams = new Map(issues.map((issue) => [issue.team.id, issue.team]));
    for (const team of teams.values()) {
      const mapped = deps.mappings.getTeamMapping(team.id);
      const conflict = deps.store.tasks.listProjects().find(
        (project) => project.prefix.toLowerCase() === team.key.toLowerCase() && project.id !== mapped?.projectId,
      );
      if (conflict)
  ...
  async function run(): Promise<LinearSyncResult> {
    ...
    try {
      ...
      const reconciled = missing.length
        ? await deps.client.issuesByIds(missing.map((mapping) => mapping.linearIssueId))
        : [];
      const reconciledById = new Map(reconciled.map((issue) => [issue.id, issue]));

      const changedProjects: string[] = [];
      const changedTasks = new Map<string, string>();
      let createdProjects = 0, createdTasks = 0, updatedTasks = 0, deactivatedTasks = 0;
      deps.store.transaction(() => {
        const teams = new Map(snapshot.issues.map((issue) => [issue.team.id, issue.team]));
        for (const team of teams.values()) {
          let mapping = deps.mappings.getTeamMapping(team.id);
          if (!mapping) {
            const project = deps.store.tasks.createProject({ name: team.name, prefix: team.key, color: DEFAULT_COLOR });
        ...
        for (const issue of snapshot.issues) {
          ...
          let mapping = deps.mappings.getIssueMapping(issue.id);
          if (mapping && mapping.linearTeamId !== issue.team.id) {
            deps.mappings.setIssueActive(issue.id, false);
            mapping = undefined;
          }
          let task = mapping ? deps.store.tasks.getTask(mapping.taskId) : undefined;
          if (!task) {
            task = deps.store.tasks.createTask({
              projectId: team.projectId, number: issueNumber(issue), title: issue.title,
              description: issue.description ?? "", status: mapState(issue), priority: priority(issue.priority), dueDate: issue.dueDate,
            });
            createdTasks++;
            ...
            const update = deps.store.tasks.updateTaskIfChanged(task.id, {
              title: issue.title, description: issue.description ?? "", priority: priority(issue.priority), dueDate: issue.dueDate,
              ...(mapping!.linearStateId === issue.state.id ? {} : { status: mapState(issue) }),
            });
            if (update.changed) { updatedTasks++; changedTasks.set(task.id, task.projectId); }
          }
          deps.mappings.upsertIssueMapping({
            linearIssueId: issue.id, taskId: task.id, linearTeamId: issue.team.id,
            identifier: issue.identifier, url: issue.url, linearStateId: issue.state.id,
            linearUpdatedAt: issue.updatedAt, active: true,
          });
        }
        ...
        for (const mapping of missing) {
          ...
          else if (issue?.state.type === "canceled" || issue?.state.type === "duplicate") terminal = "canceled";
          if (terminal && task) {
            const update = deps.store.tasks.updateTaskIfChanged(task.id, { status: terminal });
            if (update.changed) { updatedTasks++; changedTasks.set(task.id, task.projectId); }
          }
          // Missing lookup results are not evidence of deletion. Only an observed archived/unassigned issue is inactive.
          if (issue && (terminal !== undefined || issue.archivedAt !== null || issue.assignee?.id !== snapshot.viewerId)) {
            deps.mappings.setIssueActive(mapping.linearIssueId, false);
            deactivatedTasks++;
          }
        }
  ...
  return {
    sync() {
      ...
      syncing = true;
      flight = run().finally(() => { syncing = false; flight = null; });
      return flight;
    },
    getStatus() {
