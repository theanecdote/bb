...
export function registerLinearIntegration(
  ...
  const settings = bb.settings.define({
    linearApiKey: {
      ...
      description:
        "Personal API key used to synchronize assigned Linear issues.",
      secret: true,
    },
  });
  ...
  const configuredKey = async () =>
    (await settings.get()).linearApiKey?.trim() || null;
  const getClientSnapshot = async (): Promise<LinearClientSnapshot> => {
    const key = await configuredKey();
    if (!key)
      throw needsConfiguration(
        "Configure the Linear API key and reload the plugin.",
      );
    return {
      generation: clientGeneration,
      client: (client ??= createLinearClient({ apiKey: key })),
  };
  };
  const service = createLinearSyncService({
    // Resolve once at the start of a flight so key rotation cannot mix clients.
    client: async () => (await getClientSnapshot()).client,
    mappings,
    store,
    publishProjectChanged: (id) => publishProjectsChanged(bb, id),
    publishTaskChanged: (taskId, projectId) =>
      publishTasksChanged(bb, taskId, projectId),
    warn: (message) => bb.log.warn(message),
  });
  const mutations = createLinearMutationBridge({
  ...
  const sync = async (signal?: AbortSignal) => {
    ...
    if (service.rejectedCredentials())
      bb.status.needsConfiguration(
        "The Linear API key was rejected. Configure a valid key and reload the plugin.",
      );
      ...
      while (!signal.aborted) {
        ...
        if (!status.retryAt || Date.parse(status.retryAt) <= Date.now()) {
          try {
            ...
            if (!result.ok)
              bb.log.warn(
                `Linear sync failed: ${result.error?.message ?? "Unknown error"}`,
              );
          } catch (error) {
            if ((error as { name?: string }).name === "NeedsConfigurationError")
              throw error;
