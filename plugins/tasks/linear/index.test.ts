...
describe("Linear production lifecycle", () => {
  it("registers a secret, strict safe RPC, configuration status, and one service", async () => {
    ...
    ).toMatchObject({ secret: true, type: "string" });
    expect(
      harness.registrations.services.filter(
        ({ name }) => name === "linear-sync",
      ),
    ).toHaveLength(1);
  ...
  it("contains unauthorized credentials and accepts key rotation", async () => {
    ...
    const { bb, harness } = createFakePluginHost({
      settings: { linearApiKey: "secret-one" },
    });
    await plugin(bb);
  ...
  it("terminates the background service after rejected credentials without polling again", async () => {
    vi.useFakeTimers();
    const fetch = vi.fn(
      async () =>
        new Response(
    ...
    vi.stubGlobal("fetch", fetch);
    const { bb, harness } = createFakePluginHost({
      settings: { linearApiKey: "rejected-secret" },
    });
    await plugin(bb);

    await harness.runService("linear-sync").done;
